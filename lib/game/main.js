...
.requires(
	...
	'plugins.text-wrapper',
	'plugins.draw-round-rectangle',
...
.defines(function(){
	...
	MyGame = ig.Game.extend({
        name: "html5-cups-game", // Name of the game, used as part of the key name in localstorage
		...
		update: function() {
			if (ig.system.width !== this._lastSystemWidth || ig.system.height !== this._lastSystemHeight) {
				this.verticalScale = this.getVerticalScale();
				this._lastSystemWidth = ig.system.width;
				this._lastSystemHeight = ig.system.height;
			}
			...
			for (var i = 0; i < this.entities.length; i++) {
				...
				if (typeof entity.onResolutionChange === "function") {
					entity.onResolutionChange();
				}
			}
		},
		...
		getVerticalScale: function() {
			if (ig.responsive.height > ig.responsive.width) {
				return Math.min(2, ig.responsive.height/ig.responsive.width);
			} else {
				return 1;
			}
		}
